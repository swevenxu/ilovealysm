...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (topicId && isPinnacleFile(file.filename)) {
      const { data: topic } = await supabase.from('topics').select('id, name').eq('id', topicId).single();
    ...
    const { data: pages } = await pagesQuery.order('page_number');
    ...
    for (let i = 0; i < sourceItems.length; i++) {
      ...
      if ((serializedItems.length > 6000 || currentItems.length >= 3 || isLast) && serializedItems.trim()) {
        try {
          ...
          const quizRows = quizData.map((q: any) => ({
            file_id: fileId, topic_id: selectedTopicId, question: q.question,
            ...
            stem: typeof q.stem === 'string' ? q.stem : q.question,
            sub_questions: Array.isArray(q.sub_questions) ? q.sub_questions : [{ number: 1, text: q.question }],
